const products = [
  {
    id: 1,
    name: "silicon Bra",
    price: 6500.0,
    description: "Strapless bra for your comfort and confidence",
    images: ["/assets/img/silicon_bra.webp"],
    colors: ["black"],
    sizeLimits: { min: 6, max: 16 },
    category: "bra",
  },
  {
    id: 2,
    name: "Casual Leggings",
    price: 12000.0,
    description:
      "Breathable leggings for your daily activities.Elegant and bold lingerie set for special occasions.",
    images: [
      "/assets/img/leggings_1.jpg",
      "/assets/img/leggings_2.jpg",
      "/assets/img/leggings_3.jpg",
      "/assets/img/leggings_4.jpg",
    ],
    colors: ["white", "black", "green", "brown"],
    sizeLimits: { min: 6, max: 12 },
    category: "legging",
  },
  {
    id: 3,
    name: "Lingerie Set I",
    price: 25000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/sexy_lingerie_set_1.jpg",
      "/assets/img/sexy_lingerie_set_2.jpg",
      "/assets/img/sexy_lingerie_set_3.jpg",
      "/assets/img/sexy_lingerie_set_4.jpg",
    ],
    colors: ["red", "black", "white"],
    sizeLimits: { min: 6, max: 16 },
    category: "lingerie",
  },
  {
    id: 4,
    name: "Lingerie Set II",
    price: 22000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/lingerie_set_x.jpg"],
    colors: ["red"],
    sizeLimits: { min: 6, max: 16 },
    category: "lingerie",
  },
  {
    id: 5,
    name: "Lingerie Set III",
    price: 22000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/sexy_lingerie-set_5.webp"],
    colors: ["black", "red", "pink"],
    sizeLimits: { min: 6, max: 16 },
    category: "lingerie",
  },
  {
    id: 6,
    name: "Full Gym Wear I",
    price: 25000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/full_gym_wear_1.jpg",
      "/assets/img/full_gym_wear_2.jpg",
      "/assets/img/full_gym_wear_3.jpg",
      "/assets/img/full_gym_wear_4.jpg",
    ],
    colors: ["black", "pink", "green", "blue"],
    sizeLimits: { min: 6, max: 18 },
    category: "gym_wear",
  },
  {
    id: 7,
    name: "Short Gym Wear",
    price: 13500.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/short_gym_wear_1.jpg",
      "/assets/img/short_gym_wear_2.jpg",
      "/assets/img/short_gym_wear_3.jpg",
      "/assets/img/short_gym_wear_4.jpg",
    ],
    colors: ["black", "grey"],
    sizeLimits: { min: 6, max: 18 },
    category: "gym_wear",
  },
  {
    id: 8,
    name: "Full Gym Wear II",
    price: 10000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/gym_wear.jpg",
      "/assets/img/gym_wear_2.jpg",
      "/assets/img/gym_wear_3.jpg",
      "/assets/img/gym_wear_4.jpg",
    ],
    colors: ["black", "blue"],
    sizeLimits: { min: 6, max: 18 },
    category: "gym_wear",
  },
  {
    id: 9,
    name: "2-Piece Night Wear",
    price: 8500.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/2_pcs_night_wear_4.jpg"],
    colors: ["white"],
    sizeLimits: { min: 6, max: 16 },
    category: "night_wear",
  },
  {
    id: 10,
    name: "2-Piece Backnet Nightie",
    price: 13000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/2_pcs_backnet_nightie_2.jpg",
      "/assets/img/2_pcs_backnet_nightie.jpg",
      "/assets/img/2_pcs_backnet_nightie_3.jpg",
      "/assets/img/2_pcs_backnet_nightie_4.jpg",
    ],
    colors: ["white"],
    sizeLimits: { min: 6, max: 16 },
    category: "night_wear",
  },
  {
    id: 11,
    name: "3in1 Mesh Night Gown",
    price: 15000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/3_pcs_mesh_nightgown_3.jpg",
      "/assets/img/3_pcs_mesh_nightgown_1.jpg",
      "/assets/img/3_pcs_mesh_nightgown_2.jpg",
      "/assets/img/3_pcs_mesh_nightgown_4.jpg",
    ],
    colors: ["grey", "maroon", "pink", "red"],
    sizeLimits: { min: 6, max: 16 },
    category: "night_wear",
  },
  {
    id: 12,
    name: "Thong Shaper",
    price: 11000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/thong_shaper_1.jpg"],
    colors: ["black"],
    sizeLimits: { min: 6, max: 12 },
    category: "body_shaper",
  },
  {
    id: 13,
    name: "Silk Pyjamas",
    price: 15000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: [
      "/assets/img/silk_pyjamas_1.png",
      "/assets/img/silk_pyjamas_2.png",
      "/assets/img/silk_pyjamas_3.png",
      "/assets/img/silk_pyjamas_4.png",
    ],
    colors: ["purple", "skyblue", "blue", "grey"],
    sizeLimits: { min: 16, max: 38 },
    category: "pyjamas",
  },
  {
    id: 14,
    name: "Checked Pyjamas",
    price: 12000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/pyjamas_2.png", "/assets/img/pyjamas_3"],
    colors: ["beige"],
    sizeLimits: { min: 16, max: 38 },
    category: "pyjamas",
  },
  {
    id: 15,
    name: "Playboy Pyjamas",
    price: 12000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/playboy_3.png", "/assets/img/playboy_3.png"],
    colors: ["purple"],
    sizeLimits: { min: 16, max: 38 },
    category: "pyjamas",
  },
  {
    id: 16,
    name: "Cotton Bra-Top",
    price: 4500.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/s_bra_4.png"],
    colors: ["black", "white", "yellow", "maroon", "blue", "grey", "lime"],
    sizeLimits: { min: 10, max: 16 },
    category: "bra_top",
  },
  {
    id: 17,
    name: "Lace Bra-Top",
    price: 4500.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/lace_bra_top_1.png",],
    colors: ["black", "white", "navy", "grey", "maroon"],
    sizeLimits: { min: 10, max: 16 },
    category: "bra_top",
  },
  {
    id: 18,
    name: "High Waist Girdle",
    price: 12000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/high_waist_girdle_4.webp", "/assets/img/high_waist_girdle_3.jpg"],
    colors: ["black",],
    sizeLimits: { min: 10, max: 16 },
    category: "girdle",
  },
  {
    id: 19,
    name: "Thigh Tights",
    price: 5000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/tights_1.jpg",],
    colors: ["black",],
    sizeLimits: { min: 10, max: 16 },
    category: "tights",
  },
  {
    id: 20,
    name: "Tights with Pockets",
    price: 5000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/skin_tight_side_pocket_1.jpg",],
    colors: ["blue","black", "maroon", "grey", "green", "brown"],
    sizeLimits: { min: 10, max: 16 },
    category: "tights",
  },
  {
    id: 21,
    name: "Camisole Vest",
    price: 4000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/camisole_vest_1.png",],
    colors: ["black", "white", "blue", "yellow"],
    sizeLimits: { min: 10, max: 16 },
    category: "camisole",
  },
  {
    id: 22,
    name: "BDSM Kit",
    price: 30000.0,
    description: "Lorem ipsum var deux vox machina.",
    images: ["/assets/img/bdsmkit_3.jpg", "/assets/img/bdsmkit_2.jpg", "/assets/img/bdsmkit_1.jpg",],
    colors: ["black", "red", "pink",],
    sizeLimits: { min: 0, max: 0 },
    category: "tights",
  },
];
